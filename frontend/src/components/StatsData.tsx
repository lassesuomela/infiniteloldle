...
type OtherStats = {
  ...
  score_count_graph: any[];
};

export default function StatsData() {
  const [otherStat, setOtherStat] = useState<OtherStats | null>(null);
  ...
  const PlayerCountryTooltip = ({ active, payload }: TooltipProps) => {
  ...
  const ScoreDistrTooltip = ({ active, payload }: TooltipProps) => {
    if (active && payload && payload.length) {
  ...
  const PlayerTooltip = ({ active, payload }: TooltipProps) => {
    if (active && payload && payload.length) {
  ...
  const DauTooltip = ({ active, payload, label }: TooltipProps) => {
    if (active && payload && payload.length) {
  ...
  const UsersTooltip = ({ active, payload, label }: TooltipProps) => {
    if (active && payload && payload.length) {
