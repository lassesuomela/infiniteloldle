...
type StatPoint = { Dates: string; Value: number };
type ParsedStats = [StatPoint[], number];

export default function StatsData() {
  const [gamesPerDay, setGamesPerDay] = useState<StatPoint[]>([]);
  const [triesPerDay, setTriesPerDay] = useState<StatPoint[]>([]);
  const [firstTriesPerDay, setFirstTriesPerDay] = useState<StatPoint[]>([]);
  const [scoresPerDay, setScoresPerDay] = useState<StatPoint[]>([]);
  ...
  const parse = (data: string | null): ParsedStats => {
    if (data === null) {
      return [[], 0];
    }
